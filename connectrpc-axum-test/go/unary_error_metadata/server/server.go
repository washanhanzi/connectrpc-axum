package main

import (
	"context"
	"fmt"
	"log"
	"net"
	"net/http"
	"os"

	"connectrpc.com/connect"
	"github.com/connectrpc-axum/test/go/gen"
	"github.com/connectrpc-axum/test/go/gen/genconnect"
)

type unaryErrorMetadataServer struct {
	genconnect.UnimplementedHelloWorldServiceHandler
}

func (s *unaryErrorMetadataServer) SayHello(
	_ context.Context,
	req *connect.Request[gen.HelloRequest],
) (*connect.Response[gen.HelloResponse], error) {
	err := connect.NewError(connect.CodeInvalidArgument, fmt.Errorf("name is required"))
	err.Meta().Set("x-custom-meta", "custom-value")
	err.Meta().Set("x-request-id", "test-123")
	return nil, err
}

func main() {
	socketPath := os.Getenv("SOCKET_PATH")
	if socketPath == "" {
		log.Fatal("SOCKET_PATH env var is required")
	}

	mux := http.NewServeMux()
	path, handler := genconnect.NewHelloWorldServiceHandler(&unaryErrorMetadataServer{})
	mux.Handle(path, handler)

	listener, err := net.Listen("unix", socketPath)
	if err != nil {
		log.Fatalf("failed to listen on %s: %v", socketPath, err)
	}

	if err := http.Serve(listener, mux); err != nil {
		log.Fatalf("server error: %v", err)
	}
}
