...
type streamingReceiveMaxBytesServer struct {
	genconnect.UnimplementedHelloWorldServiceHandler
}

func (s *streamingReceiveMaxBytesServer) SayHelloStream(
	ctx context.Context,
...
func main() {
	...
	path, handler := genconnect.NewHelloWorldServiceHandler(
		&streamingReceiveMaxBytesServer{},
		connect.WithReadMaxBytes(64),
	)
