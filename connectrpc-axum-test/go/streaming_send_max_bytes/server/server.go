...
type streamingSendMaxBytesServer struct {
	genconnect.UnimplementedHelloWorldServiceHandler
}

func (s *streamingSendMaxBytesServer) SayHelloStream(
	ctx context.Context,
	...
	switch name {
	case "Small":
		if err := stream.Send(&gen.HelloResponse{Message: "Hi"}); err != nil {
			return err
		}
		if err := stream.Send(&gen.HelloResponse{Message: "Hey"}); err != nil {
			return err
		}
	case "Large":
...
func main() {
	...
	path, handler := genconnect.NewHelloWorldServiceHandler(
		&streamingSendMaxBytesServer{},
		connect.WithSendMaxBytes(64),
	)
