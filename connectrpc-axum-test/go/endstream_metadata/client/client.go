...
func main() {
	...
	err := runTest(client)
	if err != nil {
		fmt.Printf("    FAIL  EndStream frame contains error metadata: %v\n", err)
		os.Exit(1)
	}
	fmt.Printf("    PASS  EndStream frame contains error metadata\n")
}

func runTest(client *http.Client) error {
	url := "http://localhost/hello.HelloWorldService/SayHelloStream"

	jsonBody := []byte(`{"name":"Metadata Tester"}`)
	...
	// HTTP status should be 200 for streaming responses
	...
	var endStreamJSON map[string]any
	...
	for len(cursor) >= 5 {
		...
		if flags&0x02 != 0 {
			...
			foundEndStream = true
			if err := json.Unmarshal(payload, &endStreamJSON); err != nil {
	...
	errorObj, ok := endStreamJSON["error"].(map[string]any)
	if !ok {
		return fmt.Errorf("expected error field in EndStream, got: %v", endStreamJSON)
	}
	...
	// Validate that metadata is present in the EndStream frame
	metadataRaw, ok := endStreamJSON["metadata"].(map[string]any)
	if !ok {
		return fmt.Errorf("expected metadata field in EndStream, got: %v", endStreamJSON)
	}

	// Build a lowercase-keyed lookup since Go servers may title-case header names
	metadata := make(map[string]any, len(metadataRaw))
	for k, v := range metadataRaw {
		metadata[strings.ToLower(k)] = v
	}

	// Metadata values are arrays of strings per the Connect protocol spec
	customMetaArr, ok := metadata["x-custom-meta"].([]any)
	if !ok || len(customMetaArr) == 0 {
		return fmt.Errorf("expected x-custom-meta in metadata, got: %v", metadata)
	}
	customMeta, ok := customMetaArr[0].(string)
	if !ok || customMeta != "custom-value" {
		return fmt.Errorf("expected x-custom-meta 'custom-value', got: %v", customMetaArr)
	}

	requestIDArr, ok := metadata["x-request-id"].([]any)
	if !ok || len(requestIDArr) == 0 {
		return fmt.Errorf("expected x-request-id in metadata, got: %v", metadata)
	}
	requestID, ok := requestIDArr[0].(string)
	if !ok || requestID != "req-123" {
		return fmt.Errorf("expected x-request-id 'req-123', got: %v", requestIDArr)
	}
