...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin = build_go_binary(
        &go_dir,
        "./connect_client_stream/server/",
        "connect-client-stream-server",
    )
    .await?;
    let go_client_bin = build_go_binary(
        &go_dir,
        "./connect_client_stream/client/",
        "connect-client-stream-client",
    )
    .await?;
    ...
    println!("=== Connect Client Stream Integration Tests ===");

    let (rs_go, rs_rs, go_go, go_rs, interceptor_cases) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_client_stream_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_client_stream_tests(go_sock),
        client::run_client_stream_interceptor_tests(rust_tcp_addr),
    );
