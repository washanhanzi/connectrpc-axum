...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./tonic_extractor/server/", "tonic-extractor-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./tonic_extractor/client/", "tonic-extractor-client").await?;
