...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./endstream_metadata/server/", "endstream-metadata-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./endstream_metadata/client/", "endstream-metadata-client").await?;
    ...
    println!("=== EndStream Metadata Integration Tests ===");

    let (rs_go, rs_rs, go_go, go_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_endstream_metadata_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_endstream_metadata_tests(go_sock),
    );
