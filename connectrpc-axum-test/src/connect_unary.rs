...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin = build_go_binary(&go_dir, "./connect_unary/server/", "unary-server").await?;
    let go_client_bin = build_go_binary(&go_dir, "./connect_unary/client/", "unary-client").await?;
