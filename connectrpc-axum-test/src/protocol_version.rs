...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./protocol_version/server/", "protocol-version-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./protocol_version/client/", "protocol-version-client").await?;
