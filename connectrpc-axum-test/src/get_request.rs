...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./get_request/server/", "get-request-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./get_request/client/", "get-request-client").await?;
    ...
    println!("=== GET Request Integration Tests ===");

    let (rs_go, rs_rs, go_go, go_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_get_request_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_get_request_tests(go_sock),
    );
