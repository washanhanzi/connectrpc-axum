...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./error_details/server/", "error-details-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./error_details/client/", "error-details-client").await?;
