...
pub async fn run(rust_sock: &TestSocket, _go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_client_bin = build_go_binary(
        &go_dir,
        "./timeout_rechunked_stream/client/",
        "timeout-rechunked-stream-client",
    )
    .await?;
    ...
    // The re-chunking middleware under test lives inside the Rust server
    // stack (between the handler and ConnectLayer's deadline body wrapper),
    // so only the Rust server is exercised here.

    rust_sock.wait_ready().await?;

    println!("=== Timeout Re-chunked Stream Integration Tests ===");

    let (rs_go, rs_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_tests(rust_sock),
    );
