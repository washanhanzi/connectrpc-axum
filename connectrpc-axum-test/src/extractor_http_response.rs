...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./extractor_http_response/server/", "extractor-http-response-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./extractor_http_response/client/", "extractor-http-response-client").await?;
    ...
    println!("=== Extractor HTTP Response Integration Tests ===");

    let (rs_go, rs_rs, go_go, go_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_extractor_http_response_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_extractor_http_response_tests(go_sock),
    );
