...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./receive_max_bytes_5mb/server/", "receive-max-bytes-5mb-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./receive_max_bytes_5mb/client/", "receive-max-bytes-5mb-client").await?;
    ...
    println!("=== Receive Max Bytes 5MB Integration Tests ===");

    let (rs_go, rs_rs, go_go, go_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_receive_max_bytes_5mb_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_receive_max_bytes_5mb_tests(go_sock),
    );
