...
const TEST_CASES: &[TestCase] = &[TestCase {
    name: "EndStream frame contains error metadata",
}];
...
pub async fn run_endstream_metadata_tests(sock: &TestSocket) -> Vec<CaseResult> {
...
async fn run_one(sock: &TestSocket, _tc: &TestCase) -> anyhow::Result<()> {
    ...
    let request_body = r#"{"name":"Metadata Tester"}"#;
    ...
    let mut end_stream_json: Option<serde_json::Value> = None;
    ...
    let end_stream = end_stream_json.unwrap();

    // Validate the error in the EndStream frame
    let error_obj = end_stream
        .get("error")
        .ok_or_else(|| anyhow::anyhow!("expected error field in EndStream, got: {end_stream}"))?;
