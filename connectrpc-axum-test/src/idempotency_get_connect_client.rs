...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin = build_go_binary(
        &go_dir,
        "./idempotency_get_connect_client/server/",
        "idempotency-get-connect-client-server",
    )
    .await?;
    let go_client_bin = build_go_binary(
        &go_dir,
        "./idempotency_get_connect_client/client/",
        "idempotency-get-connect-client-client",
    )
    .await?;
    ...
    println!("=== Idempotency GET Connect Client Integration Tests ===");

    let (rs_go, rs_rs, go_go, go_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_idempotency_get_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_idempotency_get_tests(go_sock),
    );
