...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin = build_go_binary(
        &go_dir,
        "./streaming_extractor_client/server/",
        "streaming-extractor-client-server",
    )
    .await?;
    let go_client_bin = build_go_binary(
        &go_dir,
        "./streaming_extractor_client/client/",
        "streaming-extractor-client-client",
    )
    .await?;
    ...
    println!("=== Client Streaming Extractor Integration Tests ===");

    // Run all 4 client tests concurrently
    let (rs_go, rs_rs, go_go, go_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_streaming_extractor_client_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_streaming_extractor_client_tests(go_sock),
    );
