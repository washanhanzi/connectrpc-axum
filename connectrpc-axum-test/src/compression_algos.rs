...
pub async fn run(rust_sock: &TestSocket, _go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    // Go server only supports gzip, so we only test Rust server here.
    // gzip-specific tests are covered by streaming_compression_gzip module.

    rust_sock.wait_ready().await?;

    println!("=== Compression Algorithms Integration Tests ===");

    let (rs_go, rs_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_compression_algos_tests(rust_sock),
    );
