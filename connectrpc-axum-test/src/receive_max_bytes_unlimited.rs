...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./receive_max_bytes_unlimited/server/", "receive-max-bytes-unlimited-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./receive_max_bytes_unlimited/client/", "receive-max-bytes-unlimited-client").await?;
    ...
    println!("=== Receive Max Bytes Unlimited Integration Tests ===");

    let (rs_go, rs_rs, go_go, go_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_receive_max_bytes_unlimited_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_receive_max_bytes_unlimited_tests(go_sock),
    );
