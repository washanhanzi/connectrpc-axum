...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./axum_router/server/", "axum-router-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./axum_router/client/", "axum-router-client").await?;
