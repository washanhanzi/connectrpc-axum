...
pub async fn run(rust_sock: &TestSocket, go_sock: &TestSocket) -> anyhow::Result<()> {
    ...
    let go_server_bin =
        build_go_binary(&go_dir, "./streaming_error/server/", "streaming-error-server").await?;
    let go_client_bin =
        build_go_binary(&go_dir, "./streaming_error/client/", "streaming-error-client").await?;
    ...
    println!("=== Streaming Error Integration Tests ===");

    let (rs_go, rs_rs, go_go, go_rs) = tokio::join!(
        run_go_client(rust_sock, &go_client_bin),
        client::run_streaming_error_tests(rust_sock),
        run_go_client(go_sock, &go_client_bin),
        client::run_streaming_error_tests(go_sock),
    );
