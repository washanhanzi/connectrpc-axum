...
pub fn decode_hello_request_proto(bytes: &[u8]) -> HelloRequest {
    HelloRequest::decode(bytes).expect("decode HelloRequest")
}
...
pub fn decode_hello_response_proto(bytes: &[u8]) -> HelloResponse {
    HelloResponse::decode(bytes).expect("decode HelloResponse")
}
...
pub fn parse_streaming_proto_responses(bytes: &[u8]) -> usize {
    ...
    while cursor.len() >= 5 {
        ...
        let _ = HelloResponse::decode(payload).expect("decode stream frame");
        count += 1;
    }
...
async fn say_hello(
    ...
    Ok(ConnectResponse::new(HelloResponse {
        ...
        response_type: Some(hello_response::ResponseType::Details(ResponseDetails {
