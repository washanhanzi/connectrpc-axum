...
impl CallOptions {
    ...
    ///
    /// Note: Protocol-reserved headers are filtered and will not be sent.
    /// Reserved headers include those starting with `Connect-` or `Grpc-`,
    /// as well as `Content-Type`, `Content-Encoding`, `Accept-Encoding`,
    /// and `Content-Length`.
    ///
    /// # Panics
    ///
